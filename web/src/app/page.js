...
export default function Home() {
  ...
  const auGlobalCount = results.ideas
    ? results.ideas.filter(idea => idea.region?.toLowerCase() !== 'africa').length
    : 0;
  const africaCount = results.ideas
    ? results.ideas.filter(idea => idea.region?.toLowerCase() === 'africa').length
    : 0;
            ...
            {loading ? (
              ...
              <>
                ...
                <div style={{
                  ...
                  borderRadius: 14,
                  padding: '20px 24px',
                  marginBottom: filteredPreviousIdeas?.length > 0 ? 32 : 0,
                  display: 'flex',
                  alignItems: 'center',
                  gap: 14,
                }}>
                ...
                {filteredPreviousIdeas?.length > 0 && (
                  <div className="flex-col gap-8">
                    {filteredPreviousIdeas.map((idea, i) => (
                      <IdeaCard key={i} index={i + 1} idea={idea} delay={i * 0.06} />
                    ))}
                  </div>
                )}
